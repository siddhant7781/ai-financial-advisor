...
const llm_utils = require('../llm_utils');
const axios = require('axios');
const ETF_UNIVERSE = require('../data/universe.json');
...
router.get('/api/universe', (req, res) => {
...
router.post('/api/recommend', async (req, res) => {
    ...
    try {
        ...
        // Try LLM if key available and requested
        if (process.env.OPENAI_API_KEY && profile.useLLM) {
            try {
                ...
                const systemPrompt = `You are a financial education assistant. Output ONLY a valid JSON object with the following keys:\n- allocations: array of objects {ticker, weight}\n- rationale: string explaining the allocation\n- risk_notes: string with risk considerations.\nAll weights must sum to exactly 1.0. Do not include any additional text. Use the provided market snapshot and constraints when forming the allocation.`;
                ...
                const maybe = llm_utils.extractFirstJson(text);
                const universeTickers = ETF_UNIVERSE.map(e => e.ticker);
                const valid = llm_utils.validateAllocations(maybe, universeTickers);
                if (valid.ok) {
                    const norm = llm_utils.normalizeAllocations(maybe);
...
// scenario endpoint
router.post('/api/scenario', (req, res) => {
    (async () => {
